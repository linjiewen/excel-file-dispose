...
namespace yiiComponent\excelFile;
 ...
 */
class ExcelFile extends \yii\base\Component
{
    /**
     * 读取数据
     *
    ...
    /**
     * 读取office文件数据利用PhpOffice\PhpSpreadsheet组件
     *
     * @param $file
     * @return array
    ...
    public function getFileData($file)
    {
        ...
        $tableData = $spreadsheet->getActiveSheet()->toArray(null, true, true, true);
        $first = array_shift($tableData);
        $data = [];
    ...
    public function importCsv()
    {
        ...
        $data = file_get_contents(Yii::$app->request->post('url'));
        $reader = Reader::createFromString($data, 'r');
        $reader->setOutputBOM(Reader::BOM_UTF8);
        $reader->setHeaderOffset(0);
        $records = ArrayHelper::toArray($reader->getRecords());
